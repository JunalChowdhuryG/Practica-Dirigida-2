...
function mostrar_menu_principal() {
    ...
    echo "13) generar reporte de estado del repositorio"
...
function agregar_subtree() {
    ...
    read directorio
    echo -n "Ingrese la rama del repositorio externo (por defecto main): "
    read rama
    rama=${rama:-main}
...
function gestionar_diff() {
    while true; do
        ...
        echo "d) Volver al menú principal"
        echo "e) Comparar diferencias de un archivo específico"
        echo -n "Seleccione una opción: "
        read opcion_diff
        case "$opcion_diff" in
            a|A)
...
function gestionar_hooks() {
    while true; do
        ...
        echo "e) Volver al menú principal"
        echo "f) Instalar hook  verifica documentaciOn en commits"
        echo -n "Seleccione una opción: "
        read opcion_hooks
